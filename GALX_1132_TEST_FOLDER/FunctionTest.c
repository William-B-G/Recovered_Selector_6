...
*/    
	//Service             param Options    Startup text       Pass text                Fail text           Descript
	{Dsply_Msg_29_Return,  0x2000 + 1, 0x031,   "Checking CTCAN      ",  "CAN OK              ", "CAN Fail            " , "testing CTCAN "},
    {Generic_Tester,       0, 0x031,   "Checking REOPEN LEDs",  "REOPEN OK           ", "REOPEN LED Fail     " , "REOPEN LED    "},
    {Generic_Tester,       1, 0x031,   "Checking CN18       ",  "CN18 LCSE OK        ", "CN18 LCSE Fail      " , "CN18 LCSE     "},
    {Generic_Tester,       2, 0x031,   "Checking CN19       ",  "CN19 LCSE OK        ", "CN19 LCSE Fail      " , "CN19 LCSE     "},
    {Generic_Tester,       3, 0x031,   "Chkg C-002 IO Bds   ",  "IO Boards OK        ", "IO Boards Fail      " , "C-002 IO Bds  "},
    
    {Generic_Tester,       4, 0x031,   "Checking DCL        ",  "DCL       OK        ", "DCL Fail            " , "DCL           "},
    {Generic_Tester,       5, 0x031,   "Checking RO         ",  "RO        OK        ", "RO  Fail            " , "RO            "},
    {Generic_Tester,       6, 0x031,   "Checking DOL        ",  "DOL       OK        ", "DOL Fail            " , "DOL           "},    
   	
    {Generic_Tester,       7, 0x031,   "Checking CLOSE SW2  ",  "CLOSE SW2 OK        ", "CLOSE SW2 Fail      " , "Manual Cls SW2"},
    {Generic_Tester,       8, 0x031,   "Checking OPEN SW2   ",  "OPEN SW2 OK         ", "OPEN SW2 Fail       " , "Manual Opn SW2"},
    {Generic_Tester,       9, 0x031,   "Checking NUDGE SW3  ",  "NUDGE SW3 OK        ", "NUDGE SW3 Fail      " , "Manual Ndg SW3"},
    {Generic_Tester,       10, 0x031,  "Checking HEAVY SW4  ",  "HEAVY SW4 OK        ", "HEAVY SW4 Fail      " , "Manual Hvy SW4"},	
    
    {Generic_Tester,       11, 0x031,   "Checking J1 IN      ",  "J1 IN OK            ", "J1 IN Fail          " , "Jmpr JP1      "},
    {Generic_Tester,       15, 0x031,   "Checking J1 OUT     ",  "J1 OUT OK           ", "J1 OUT Fail         " , "Jmpr JP1      "},
    {Generic_Tester,       14, 0x031,   "Checking J5 OUT     ",  "J5 OUT OK           ", "J5 OUT Fail         " , "Jmpr JP5      "},
    {Generic_Tester,       13, 0x031,   "Checking J4 OUT     ",  "J4 OUT OK           ", "J5 OUT Fail         " , "Jmpr JP4      "},
...
/*******************************************************************************
  ...
  Summary:
   Tests are controlled by the data in one of two arrays: 
   Functional_Menu_A[] or Functional_Menu_B[]. These arrays are of type:
   struct functions_to_test. The arrays contain the instructions of what
   tests to run and the param to send to the test. These tests can be 
   specific tests or can be generic tests in which the param is an index
   into an array holding information about the generic test.
     
  Author:
    William Grauer
    
  Description:
   Calling is of the form: FunctionalTest(X + Y), where X determines the
   array (A or B) and Y determines how to react to a failure or to test only
   the test in the array that have failed. See 'Parameters' below.

   The items in the structure for a test in the array is defined in
   'struct functions_to_test'. Contains a function ptr to the test, the single
   parameter to send to the test, various options that are used by
   Functional_Menu_x[] and some text msgs.

   The test function ptr is called in case PerformTheTestCase: of 
   FunctionalTest(z);

   The information as to whether the test has been previously run; has passed
   or failed, is contained in the 'OptionsMask' of 'struct functions_to_test'

  Preconditions:
    One or both of Functional_Menu_A[] or Functional_Menu_B[] must contain
    data and the data must have pointers to existing tests.

  Parameters:
   Calling is of the form: FunctionalTest(X + Y).
   If X = 0, Functional_Menu_A[] will be used.
   If X = 0x100, Functional_Menu_B[] will be used.
   If Y = 1, Test all items in array but stop testing on the first failure.
   If Y = 2, Test only the tests that previously failed.
   If Y = 3, Test all items in array and continue testing even if a
      test has failed.
  ...
  Remarks:
   The purpose of the two arrays: Functional_Menu_A[] or Functional_Menu_B[],
   is to facilitate SW dev and debugging so that a subset of tests can be
   run by having two arrays and switching between them by calling with
   param 'X' = 0 or 1.
...
// Functional_Menu[test_index].OptionsMask
    ...
    case DoneWith_FT:
        FunctionTestCase = InitFunctionTest;
        clr_lcd_dply(); // clear 4-line display
        // Dismiss any old "ENTER" or "MODE" buttons
...
//ZYM 2-2-2018        
        ...
        else if(StopOnFail==1)
        {
        	...
        	if(CurrentTestPassFail) // 1 == test passed
        	{
        		New_write_display("NO FAULT            ",3,0);	
        	}        	
        }
    	...
    	if(Test_failures)
    	{
    		sprintf(src,"DONE - %d Failures", Test_failures);
    		New_write_display( src,1,0);
    		New_write_display( "See Msg Brd C 4 info",2,0);	
    	}
    	else New_write_display("DONE - All PASS     ",1,0);             
...
void SaveTestResult(int16 test_index)
{
    ...
    {
        CurrentTestPassFail = PASS; //  To handle WIFI test Stop (bit 6 of param)
        return;
    }
