...
*    retest.                                  *
*                                             * 
* Timing explained: The timer ISR is set for  * 
*   10ms. In main(), when the 10ms ISR fires  *
*   main() will fall into the Master Loop.    * 
*   This Master Loop thus executes every 10ms.* 
*   The Master Loop will call .new_inctime()  *
*   new_inctime(), increments most of the     *
*   timers (above #7) every 100ms.  These     * 
*   100ms timers are used for time delays in  * 
*   The tests. So, if the test sets 'Delay'=5 *
*   That will cause a test to pause for 500ms.*
*                                             * 
*   The menu manager is called from main(),   * 
*   but only every other time thru Master Loop* 
*   Thus, menu manager is called every 20ms   *
*   For the case where a test has a Delay=5,  * 
*   (which is 500ms), the menu manager has    * 
*   called the test code (500ms/20ms) 25 times*
*                                             *
*                                             * 
*                                             * 
*                                             *
...
//        SIU.GPDO[Z6_LED].R = !SIU.GPDO[Z6_LED].R; // toggle LED
        SIU.GPDO[Z6_LED].R = 1; // turn OFF
        break;

    case LookAtButtons:
//        SIU.GPDO[Z6_LED].R = 1; // OFF // disable this line for DEBUG of Select 6 CTCAN. WBG 2019-1-03
...
int16 ServiceTheButtons(void)
{
    ...
    if (Check_for_Mode_Button())
    {
    	New_Home_Sel_6_APS = Sel_6_APS;  // Home the APS for Selector 6
        //-     SIU.GPDO[Z6_LED].R = 1;	// turn led OFF (wbg)// tagwbg 15
        New_LCD_Menu = Init_the_Menu;
...
int16 Handle_Retest_Request(int16 The_Kase)
{
	...
	if(Global_Allow_Retest == NO) return 0; // no retest allowed.
