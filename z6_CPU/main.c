...
int16 Global_Allow_Retest = YES;
int16 GenericTest_index = 0; // a global index for accessing the Generic_test_params struct.
int16 Read_All_inputs_Control_byte = 0;
// Read_All_inputs_Control_byte.bit0 => 0 disable substitution with 'assert_....' values.
// Read_All_inputs_Control_byte.bit1 => 0 Use 'Assert_initial_read' as the substitute.
// Read_All_inputs_Control_byte.bit1 => 1 Use 'Assert_final_read' as the substitute.
// Read_All_inputs_Control_byte.bit15..2 : unused.
...
char src_B[25];
char src_C[25];
uint8 Button_Pressed[4];  // Button pressed
...
uint8 Enable_CT_CAN_Ctrl = 1; // [1 = enable ]. Disable while doing Car Top CAN test (wbg)
...
int16 Global_Door = 0; 
            // Global_Door = 0 ==> Front door.
            // Global_Door = 1 ==> Rear door. 
int16 SimulateCamera = 0; // 1 = simulate the camera on the selector 6 box            
...
/*******************************************************************************
* VERSION
* to see where the short version info comes up search for 'Version_Display'
* and 'SW_Vers_disp'
*******************************************************************************/
const int8 version =  0;  // tagwbg 11 - version Now
const int8 revision = 0;
const int8 interim_revision = 1;  // interim revision valid 0 - 255
const int8 Sandbox_revision = 'F' ;  // Normally, set this = '0'. 
  // Then increment as 'A', 'B', etc    

////////////////////   

//char *VerDate = "2018-11-26 @9:44";  //  'A' 
// A short version info is shown on line 0 every 2s and alternates between
// the short version info and the name of the current menu. Both display information
// shares the first line of the display and is handled in the MenuManager - Service buttons case.
// Search for 'Version_Display' and for 'SW_Vers_disp'.
// Happens in ServiceTheButtons in MenuManager.c

//char *VerDate = "2019-1-02 @13:54";  //  'A' 
//char *VerDate = "2019-1-14 @7:23";  //  'B' 
//char *VerDate = "2019-1-14 @11:12";  //  'C' 
//char *VerDate = "2019-1-21 @6:48";  //  'D'
//char *VerDate = "2019-1-22 @7:41";  //  'E'
char *VerDate = "2019-1-23 @10:58";  //  'F'
// 'B' - Ready for production use. Performs Function test on APS camera, and DTS, UTS magnetic
// switches. Does not test 1133 bd because that was already tested with the 1133 test fixture.

// 'C' - Works with 'New_APS_Delta'. 7-ZIP & .git uploaded to AWS on 2019-1-20.
// 'D' - Add detection of msgs from SPI CAN.
// 'E' - Have 'Functional Test' determine if SPI CAN is working. Installed in shop.
// 'F' - TBD

// Derived from F:\repos\Generic_1132_based_tester, ver 0.0.0.B (2019-1-02)
...
// 	Enable_gr_CAN_Ctrl = 0; // disable group CAN for 1137 / 1139 test // tagwbg 23 - REVISIT 2019 WBG
 	The_120VAC_Ptr = (void *)SEB_outputs_array[O_C120T_enum];
// 	Set_or_clear_OPL_PWR(1); // power up the OPL-0106 door operator board
...
//            setgoutp(GRtest_osignal_array[7]); 
          ...
          /////// START OF Master Loop (every 10ms)   ///////////////////        
          Timer_tic = 0;  // is set in the timer ISR (wbg)     
///////            
                       
          new_inctime(); // increment all SW timers.
// See "void MenuManager(void)" for a description of the overall timing.                      		  
          ...
          {	// enter here every 10 ms for "if(1)", or 20ms with "if(timers[tinc] & 0x01)"    
              ...
              MenuManager(); // the init_control() calls init_LCD() which 
          	    ...
          	    // MenuManager() called every 20ms now (but the Delay timers in 'Tests' are
          	    // incremented every 100ms. 
// See "void MenuManager(void)" for a description of the overall timing.          	           	  
              LCD_Buffer_to_spi(); // write the buffers to the display via SPI 
              // See Monitor_Variables.c to see how the Dyn_var_x displayed.
                  Dyn_var_1++; // for testing the generic variable display tool.
                  Dyn_var_2--; // for testing the generic variable display tool.
//                  Dyn_var_3++; // for testing the generic variable display tool.
//                  Dyn_var_4--; // for testing the generic variable display tool.
                                                    		                    	;
          }      	  	        	       			    	    		    	
  	} // END OF "for (;;)" 
} // END OF "main()"
 ...
 *
 *
 *  v 0.0.1 - A (2019-1-02) - Start Selector-6 test fixture project. 
 *     
 * =======================================================================
...
Clear All Faults(7); // clear msg boards A(1); B(2); C(4);    
