...
void *GALX4_0047_OUT_array[GALX4_0047_index] =
    {
        O_LDO6, // label on GALX4_0047 says IO7, and this is [0].
        O_LDO7,
        O_LDO8,
        O_LDO9,
        O_LDO10,
        O_LDO11, // label on GALX4_0047 says IO12, and this is [5].
};
...
int16 toggle_OPL_0106(int16 dummy) 
{
    OPL_0106 = !OPL_0106;
    return FINISHED_WITH_SERVICE;
}
...
//        setoutp(GALX4_0047_OUT_array[1]);
        if (rdinp(GALX4_0047_in_array[0])) // switch 1 FOR CTCAN
        {
            setoutp(GALX4_0047_OUT_array[0]); // FOR CTCAN
            SIU.GPDO[Z6_LED].R = 1; // OFF
        }
        else
        {
            clroutp(GALX4_0047_OUT_array[0]); // FOR CTCAN
            SIU.GPDO[Z6_LED].R = 0; // ON               
        }
    }
...
// FOR GROUP CAN -
/*
        if (rdginp(GALX4_0047_in_array[0])) // switch 1 // FOR GRCAN
        {
            setgoutp(GALX4_0047_OUT_array[0]); // FOR GRCAN
            SIU.GPDO[Z6_LED].R = 0; // ON
        }
        else
        {
            clrgoutp(GALX4_0047_OUT_array[0]); // FOR GRCAN
            SIU.GPDO[Z6_LED].R = 1; // OFF               
        }
//    if( rdginp( (void *)GRCAN_inputs_array[DTS_INDEX])  ) SIU.GPDO[Z6_LED].R = 0; // ON
//    else SIU.GPDO[Z6_LED].R = 1; // OFF  
*/
       
/*        
    if (OPL_0106) // toggled via a menu operation.
    {                
//		Msg_28_Request = 1; // request main mpu to send msg 28 to door board.
//		if(Global_Door == 0)DOOR_can.reflect_data = 1; // tell ctcan control to send the msg 28
//		  else RDOOR_can.reflect_data = 1;
...
/*
int16 Toggle_LDOxx(int16 data_zero) // This code not currently used.
{
