...
extern int16 Msg_8_Sent;
extern int16 Msg_8_Request;
extern int16 SimulateCamera;
extern int16 Reset_APS_Home_Flag;
extern uint32 Sel_6_APS;
extern int16 APS_LR_Alignment ;
extern int16 APS_FarNear_Alignment ;
extern int16 APS_Warning ;
extern int16 APS_Error ;
extern int16 APS_Normal_Max ;
extern int16 APS_FarNear_Alignment_Found;
extern int16 APS_LR_Alignment_Found;

extern int16 APS_mhb ;
extern int16 APS_mlb ;
extern int16 APS_lb;
extern int16 New_APS_Delta;
extern int16 New_APS_Delta_mm;
extern uint32 New_Home_Sel_6_APS;

int16 Msg_9_recvd;
uint32 Start_of_APS_tape = 0;
int16  APS_delta = 0;  			// from start of tape
int16  APS_Seq   = 0;
int16  APS_27_0  = 0; 				// Byte 0, addr 27
int16  APS_27_1  = 0; 				// Byte 1, addr 27
int16  APS_27_2  = 0; 				// Byte 2, addr 27	
...
COMMDWORD_TYPE rcv_APS_dword;


int16 Baud_Test = 0; //when doing a BAUD test, set this to 1 to prevent the
// detected errors from resetting the BAUD rate.

/*
	FOR THE SELECTOR 6 TEST FIXTURE, THE CAR TOP CAN (CTCAN) IS BEING USED TO ACCEPT THE CAN TRAFFIC
	FROM THE CEDES APS, AT 250kb. FOR A DESCRIPTION OF THE NORMAL USAGE OF THE CTCAN, SEE THE
	NOTES AT THE TOP OF THE FILE ctcan.c FROM ONE OF THE OTHER PROJECTS; SUCH AS THE 
	"Generic_1132_based_tester".														
...
void pack_util_ctcan(unsigned char msg, unsigned char devnmb,unsigned char tx_req, unsigned char len, unsigned char one_tx);
...
int16 msg_29_Rx = 0; // WBG (2018-10-12)
	...
	*/
	...
	if (ChipFreezeMode(c_Freeze_Start) == 1)
	{
	    ...
	    /*	
	    	...
	    	or
	    	Prescale Value = 108.75 Mhz / (250000 * 16) = 27.1875
	    	or
	    	Prescale Value = 108.75 Mhz / (57600 * 16) = 118
	    */
	    	    
	    // tagwbg 24 - CTCAN BAUD 2019 WBG	    
...
*/        
        CT_CAN.CTRL.B.PRESDIV = 26;		// Prescale Devision Factor + 1 = Prescale Value for 250000 // WBG 2019-1-02
...
void unpack_ctcan_in (void) // FOR SELECTOR 6 FIXTURE TEST add msg 26 & 27. WBG 2019-1-03
{
	int16 i;
	int16 devnmb = 0;
	int16 devix = 0;
  ...
  //  SIU.GPDO[Z6_LED].R = !SIU.GPDO[Z6_LED].R;  // toggle LED wbg  // wbg_01_2018_4_25 
	...
	{	    
		...
		{		
		    // ok at this point. SIU.GPDO[Z6_LED].R = 0; // Turn on as part of DEBUG of Selector 6 test. WBG 2019-1-03
...
/*			    
                ...
                sprintf(src,"msg=%d ix=%d len=%d",ctcan_rx[ctc_rgix].msg, ctc_rgix, ctcan_rx[ctc_rgix].len);
                Log_an_event(src); 
                sprintf(src,"id2=%d id3=%d, adr=%d",ctcan_rx[ctc_rgix].id2, ctcan_rx[ctc_rgix].id3, ctcan_rx[ctc_rgix].addr);
                Log_an_event(src);                 
*/                              
               // SIU.GPDO[Z6_LED].R = 0; // ctcan_rx[ctc_rpix].id3
              ////////////////////////////////////////			
			
			
	 			switch (ctcan_rx[ctc_rgix].addr)
				{ // THIS IS ONLY FOR THE SELECTOR 6 TEST FIXTURE TEST.
			
					// FOR SELECTOR 6 FIXTURE TEST add msg 26 & 27. WBG 2019-1-03	
					case 26:  // Selector 6 - GALX-1133
//						SIU.GPDO[Z6_LED].R = 0; // Turn on as part of DEBUG of Selector 6 test. WBG 2019-1-03
						APS_Seq = ctcan_rx[ctc_rgix].data[0];
						rcv_APS_dword.byte[hb]  = 0;
						APS_mhb = rcv_APS_dword.byte[mhb] = ctcan_rx[ctc_rgix].data[1];
						APS_mlb = rcv_APS_dword.byte[mlb] = ctcan_rx[ctc_rgix].data[2];
						APS_lb = rcv_APS_dword.byte[lb]  = ctcan_rx[ctc_rgix].data[3];
						Sel_6_APS = rcv_APS_dword.dword;
						if(Sel_6_APS < 0xFFFFFF) New_APS_Delta = New_Home_Sel_6_APS - Sel_6_APS;
						New_APS_Delta_mm = New_APS_Delta/20; // divide by 20 to get mm

						APS_LR_Alignment = ( ctcan_rx[ctc_rgix].data[7] & 0x1C ) >> 2;
						APS_FarNear_Alignment = ( ctcan_rx[ctc_rgix].data[7] & 0xE0 ) >> 5;
						if(APS_LR_Alignment) APS_LR_Alignment_Found = 1;
						if(APS_FarNear_Alignment) APS_FarNear_Alignment_Found = 1; 
						if(!APS_LR_Alignment && !APS_FarNear_Alignment ) APS_Normal_Max = Sel_6_APS; // save the max value before getting alignment warnings.
/*						
						if( !SimulateCamera)
						{
    						if(Start_of_APS_tape == 0)
    						{
    						...
    						{
    						    APS_delta = Start_of_APS_tape - rcv_APS_dword.dword;
    						}						    
						}
						if(Reset_APS_Home_Flag)
						{
    						    Start_of_APS_tape = rcv_APS_dword.dword;
    						    APS_delta = 0;
    						    Reset_APS_Home_Flag = 0;							
						}
*/						
						ctcan_dev[devnmb].rx_cnt++;
						break;						
					case 27:  // Selector 6 - GALX-1133
//						SIU.GPDO[Z6_LED].R = 0; // Turn on as part of DEBUG of Selector 6 test. WBG 2019-1-03
...
//************************************************************
//*********  Pack Utility to set up data struct **************
//************************************************************
...
void pack_util_ctcan(unsigned char msg, unsigned char devnmb,unsigned char tx_req, unsigned char Len, unsigned char One_tx)
{
				...
				ctcan_tx[ctc_tpix].len = Len;
				ctcan_tx[ctc_tpix].ex_id = 1;		// Extended Id
				ctcan_tx[ctc_tpix].one_tx = One_tx;	   // ok to retransmit 
...
void pack_ctcan_out(unsigned char msg, unsigned char devnmb,unsigned char tx_req)
{
    ...
    return; // For Select 6 test, no need to send CTCAN data.  WBG 2019-1-03
            // This 'return' is in pack_ctcan_out & tx_ctcan_msg
	...
	else switch (msg)
	{
		case 1:			// Init Safety processor Board OR load weigher
				...
				(devnmb == c_ctcan_SEL) || (devnmb == c_ctcan_DOOR) || (devnmb == c_ctcan_RDOOR) )
			{
				Len = 3;
				Tx_req = 0;
				pack_util_ctcan(msg, devnmb, tx_req, Len, Tx_req);
/*				
				ctcan_tx[ctc_tpix].rtr = 0;
				ctcan_tx[ctc_tpix].srr = 1;
		...
		case 11:		// Init data
			if ((devnmb >= c_ctcan_SEB_1) && (devnmb <= c_ctcan_END))
			{
				...
				if (ctcan_dev[devnmb].online == 1)
					Tx_req = 0;	   // ok to retransmit since device is on line
				else
					Tx_req = 1;				
				pack_util_ctcan(msg, devnmb, tx_req, Len, Tx_req);
...
*/	
		case 28:		// Door Control message
			...
			if ( (devnmb == c_ctcan_DOOR) || (devnmb == c_ctcan_RDOOR) )
                pack_util_ctcan(msg, devnmb, tx_req, Len, Tx_req);	
...
void ctcan_control (void)
{
	...
	if (nmb_ctcan_dev > 0)
	{
		unpack_ctcan_in(); // FOR SELECTOR 6 FIXTURE TEST add msg 26 & 27. WBG 2019-1-03
...
//		}
		...
		{		// The CAN bus is off
			...
			bus_off_cnt++;
			ctcan_error |= 0x04;
		    if(Baud_Test == 0)	init_ctcan_bus(); // if we are doing a BAUD test
		    //  do not init the ctcan Baud rate until done with test. (WBG 2018-10-22)
...
/*			if (((rdinp(i_ATT) == 1) && (cons[att] == 1)) || (cons[att] == 2) ||
		...
		while ((i < nmb_ctcan_dev) && (IsTxBufFul() == 0))
		{
			...
			{	// Serial Door operators
				if (devnmb == c_ctcan_DOOR)
				{
//					set_door_command(); // out by WBG (2018-10-15)
					msg_req = ctcan_get_dev_msg_req(devnmb);
					if (msg_req != 0)
					{
...
int16 ctCAN_BAUD(int16 Which_Rate)
{
	if(Which_Rate == 0) // 115.2
	{
	    CT_CAN.CTRL.B.PRESDIV = 58; // Prescale Devision Factor + 1 = Prescale Value for 115200  
	    sprintf(src, "115,200 BAUD"); 
	    Baud_Test = 0;
	}
	else 
	{ // 57.6
	    CT_CAN.CTRL.B.PRESDIV = 117; // Prescale Devision Factor + 1 = Prescale Value for 57600 
	    sprintf(src, "57,600 BAUD");
	    Baud_Test = 1; // to prevent a call to init_ctcan_bus.
	}
