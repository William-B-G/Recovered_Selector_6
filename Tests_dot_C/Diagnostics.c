...
extern int16 ctCAN_BAUD(int16);
extern int16 Global_Door;

void* UnFirePtrs[MaxFirePtrs];
int16 UnFirePtrs_index = 0;
int16 IFL_IFB_debug = 0;
int16 Msg_8_Sent = 0;
int16 Msg_8_Request = 0;
int16 Msg_28_Request = 0;
...
int16 Set_or_clear_OPL_PWR(int16 On_Off)
{
    if(On_Off) setoutp(O_PWR);
    else clroutp(O_PWR);
    return FINISHED_WITH_SERVICE; 
}
...
// Construct_Msg_28
int16 Construct_Msg_28(int16 Door_Command)
{ // construct "DOOR_can.x"; set flag to send a message
	DOOR_can.command = Door_Command;
	DOOR_can.reflect_data = 1; // tell ctcan control to send the msg 28
	
	RDOOR_can.command = Door_Command;
	RDOOR_can.reflect_data = 1; // tell ctcan control to send the msg 28	

    return FINISHED_WITH_SERVICE; 
}

// write msg 29 data to msg board A
int16 Write_msg_29_data_to_msg_board_A(int16 TheTest)
{
//      New_write_display("                    ",0,0); // row, col
	if(TheTest == 0) sprintf(src,"msg 29 Response:");
		else sprintf(src, "Comm CAN tst PASS:");

	Log_an_event(src); // to A
	// DOOR_can.status_1
	// DOOR_can.command_ret
	if(msg_29_Rx == 1) sprintf(src,"FRONT DOOR");
	  else sprintf(src,"REAR DOOR");
	Log_an_event(src); //  to A 
	// Global_Door
	sprintf(src, "Global=%d", Global_Door);
	Log_an_event(src); // to A		
		
	sprintf(src, "status 1=0x%X", DOOR_can.status_1);
	Log_an_event(src); // to A
	sprintf(src, "status 2=0x%X", DOOR_can.status_2);
	Log_an_event(src); // to A
	sprintf(src, "cmnd Retrn=0x%X", DOOR_can.command_ret);
	Log_an_event(src); // to A
	
    return FINISHED_WITH_SERVICE;			
}

int16 Front_Door_Test(int16 params)
{
    return Dsply_Msg_29_Return(1); 
}

int16 Rear_Door_Test(int16 params)
{
    return Dsply_Msg_29_Return(0x4001); 
}

//Dsply_Msg_29_Return
int16 Dsply_Msg_29_Return(int16 Params_and_Which_test)
{
	static int16 msg_29_Kase = 0;
	static int16 Retest_Option_Parm = 0; // Required for Retest Option
	static int16 RetestResponse = 1; // Required for Retest Option
//	int16 Baud_Rate = Params_and_Which_test & 0x8000; // 0 = 115.2, else 57.6
	int16 Front_Rear = Params_and_Which_test & 0x4000; // 0 = Front, else Rear
	int16 Which_test = Params_and_Which_test & 0x0ff;
	int16 Show_test_pass_msg = Params_and_Which_test & 0x2000; // 0 = show msg otherwise no msg on pass
	int16 Pass_is_Fail = Params_and_Which_test & 0x1000; // 1 = pass/fail is Fail/Pass
	// This is used to determine that jumper 4 is working because it sets the OPL to a
	// lower baud rate and the CAN Comm test will fail, but this means the jumper is working.
	    // no msg for pass.
	int16 ReturnVal = CONTINUE_SERVICE;
	
	switch(msg_29_Kase)
	{// As part of "Periodic_Background_Task", every 20ms, while not doing anything else,
	 // the "DOOR_can.reflect_data" and "RDOOR_can.reflect_data" is set to 
	 // force a msg 28 out which will cause
	 // a msg 29 to be returned.
		case 0: // init
			clr_lcd_dply();
			Clear_Msg_Board(); // A			
		    Delay_Count = 5; // 5 = 1/2s 
        	timers[t_Generic_Delay_4] = 0;
        	CurrentTestPassFail = 0;  // FAIL
        	Retest_Option_Parm = 0; 
        	RetestResponse = 1;
        	Global_Door = 0;
//        	Baud_Rate = Params_and_Which_test & 0x8000; // 0 = 115.2, else 57.6
//        	Front_Rear = Params_and_Which_test & 0x4000; // 0 = Front, else Rear
//        	Which_test = Params_and_Which_test & 0x0ff;
//        	if(Baud_Rate == 0) ctCAN_BAUD(0); else ctCAN_BAUD(1);
        	if(Front_Rear == 0) Global_Door = 1; else Global_Door = 2; 
        	// Global_Door = 1 ==> Front door.
        	// Global_Door = 2 ==> Rear door.
        				
			if(msg_29_Rx > 0)
			{ // a msg 29 already came in so send a msg 28 to
			  // cause another msg 29 to echo.
			  
			    Construct_Msg_28(0); // an empty msg.
			        // uses "DOOR_can.reflect_data" to force a msg 28 out.
				msg_29_Rx = 0;	
			}
			msg_29_Kase = 2;
			break;
/*			
		case 1: // wait for msg 29
			if(msg_29_Rx == 0)
			{
				msg_29_Kase = 2;	
			}			
			break;
*/						
		case 2: // wait for msg 29 or for time out
			if( (timers[t_Generic_Delay_4] > Delay_Count) || (msg_29_Rx > 0) )
			{

				if(msg_29_Rx == Global_Door)  
				{	
				    sprintf(src, "Param+=0x%X", Params_and_Which_test);
	                Log_an_event(src); // to A				
					Write_msg_29_data_to_msg_board_A(Which_test);
					if(Pass_is_Fail)
					{
					    msg_29_Kase = 4; // reverse the pass/fail to fail/pass   
					}
					else msg_29_Kase = 3; // a real pass	
				}
				else
					if(Pass_is_Fail)
					{
					    msg_29_Kase = 3; // reverse the pass/fail to fail/pass   
					}
					else msg_29_Kase = 4; // a real fail
			}			
			break;	
			
		case 3: // continue to display msg board A
		    if(Show_test_pass_msg == 0)
		    { // show the pass results
			...
			else
			{
				CurrentTestPassFail = 1; // PASS
			//	New_write_display("                    ",0,0); // row, col
				New_write_display("CAN OK              ",0,0); // row, col
				msg_29_Kase = 0;			
			    ReturnVal = DONE_BUT_CONTINUE_DISPLAY; //FINISHED_WITH_SERVICE;
			    // DONE_BUT_CONTINUE_DISPLAY
			}
			
			if(ReturnVal == FINISHED_WITH_SERVICE)
			{ // restore BAUD rate to 115.2
//				ctCAN_BAUD(0);
				CurrentTestPassFail = 1; // PASS
				msg_29_Kase = 0;	
			}
		...
		case 4: // time out failure	
			New_write_display("Msg 29 never recvd  ",1,0); // row, col
			CurrentTestPassFail = 0; // FAIL		
            RetestResponse = Handle_Retest_Request(Retest_Option_Parm);
            Retest_Option_Parm = 1;
            if(RetestResponse == 0) msg_29_Kase = 5;  // do not retest
            else if(RetestResponse == 1) msg_29_Kase = 0; // test again
                else ReturnVal = CONTINUE_SERVICE;    
           						
			break;	
			
		case 5: // Test fail
			New_write_display("Msg 29 FAILURE      ",2,0); // row, col			
            msg_29_Kase = 0;
            RetestResponse = 0;
            // Required for Retest Option
...
int16 CAN_Comm_Tst_Wrapper( int16* array_1, int16 param_A, int16 param_B)
{
	static int16 ReturnVal = CONTINUE_SERVICE;
	int16 TestResult = 0; // FAIL 	
	
	Global_Allow_Retest = NO; // Do not want the called test to be able to retest itself,
		// let the Generic tester do the retest.
	ReturnVal = Dsply_Msg_29_Return( param_A); // param_A will determine BAUD rate and
		// which door. See  "GenericTests[Test].int_Param_2A"
	if( rdinp(GALX4_0047_in_array[1]) ) CurrentTestPassFail = FAIL; // force a failure if switch SW1 is on (GALX4-0047 IO board)  
	if(CurrentTestPassFail) ReturnVal |= 0x8000; // pass 
	Global_Allow_Retest = YES; // Turn back on so that Generic tester can do the retest.	
	return ReturnVal;
}

int16 Baud_Tst_Wrapper( int16* array_1, int16 param_A, int16 param_B)
{
	static int16 ReturnVal = CONTINUE_SERVICE;
	int16 TestResult = 0; // FAIL 	
	...
	if( rdinp(GALX4_0047_in_array[1]) ) CurrentTestPassFail = FAIL; // force a failure if switch SW1 is on (GALX4-0047 IO board).  
	if(CurrentTestPassFail) ReturnVal |= 0x8000; // pass 
	// this is reversed in "Dsply_Msg_29_Return" because with the jumper 4 removed, the baud rate mismatch will
	// prevent the msg 29 coming in and will show a failure. But the failure means that
	// the jumper works. The reversal takes effect because the parameter is set to 0x1000
	Global_Allow_Retest = YES; // Turn back on so that Generic tester can do the retest.		
	return ReturnVal;
}
...
int16 Set_Clear_all_4_0047(int16 On_Off)
{
    int16 i = 0;
    for(i=0;i<6;i++)
    {
        if(On_Off) setoutp(GALX4_0047_OUT_array[i]);
          else clroutp(GALX4_0047_OUT_array[i]);
    }
    return FINISHED_WITH_SERVICE;
}

int16 Display_msg28_Cmnd(int16 dummy)
{
    clr_lcd_dply();
    sprintf(src, "DOOR cmnd=0x%X", DOOR_can.command);
    New_write_display(src, 1, 0);
