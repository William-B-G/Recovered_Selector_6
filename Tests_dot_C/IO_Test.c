...
#define IGNORE_PATTERN 0xffffffff // if this is the key for ignoring the expected pattern.
...
void Read_All_inputs(int16 ShowResults)
{
    ...
    Assert_Pattern = Testing_via_substitution(Read_All_inputs_Control_byte);
    if(  Assert_Pattern != 0xffffffff)
    {// returns other than 0xffffffff if we can make a substitution for a SW test.
        All_inputs = Assert_Pattern; // this is for testing that the SW will determine
            // the correct problem.  
    }
    else
    {
...
{ // Eval All_inputs against the expected pattern stored in 'Expected_Pattern[]'
  ...
  return Evaluate_The_Results(The_Expected_Pattern, (uint16)Which_IO_and_Options);       
}

//////////////////////////////////////

int16 Evaluate_The_Results(uint32 The_Expected_Pattern, uint16 Which_IO_and_Options )
{
  int16 Eval_idx = 0;
//  int16 RetrnVal = PASS;
    // Which_IO_and_Options
  uint16 Which_IO = Which_IO_and_Options & 0x00FF; // use only bits 7 thru 0
  uint16 Option = Which_IO_and_Options & 0x0C000; // options in bits 15,14 
  uint32 cmpar_Actual, cmpar_expected, Mask = 0;
 
//  setoutp(GALX4_0047_OUT_array[0]); // TO help debug a problem. LD06 (IO7)
  if(The_Expected_Pattern == IGNORE_PATTERN ) return PASS; 
// TODO wbg i_SWi0_enum
    Option = Which_IO_and_Options & 0x0C000; // options in bits 15,14   
    
     cmpar_Actual = cmpar_expected = Mask = 0;
     
       // SW debug in order to create the table for expected patterns
     if(All_inputs == The_Expected_Pattern ) return PASS; 
     else      
     {// discern the problem. Could be a short or an open
     	for(Eval_idx = 0; Eval_idx <= open_enum; Eval_idx++ )
     	{  // see which bits are different
     	    ...
     	    { // There is a difference, so determine if it is a short or an open.
     	        if(cmpar_Actual > cmpar_expected)
     	        {  // There is a short, or IO should be OFF
//     	            sprintf(src,"%s s %s", IO_Labels[Eval_idx], 
//     	                IO_Labels[Which_IO + O_IFL_COPo0_lable_enum]);
                    sprintf(src,"%s should b off", IO_Labels[Eval_idx] );
                    Log_an_event(src);     	        	
     	        }
     	        else
     	        { // There is an open, or IO should be ON
//     	             sprintf(src,"%s O %s", IO_Labels[Eval_idx], 
//     	                IO_Labels[Which_IO + O_IFL_COPo0_lable_enum]);
                    sprintf(src,"%s should b on", IO_Labels[Eval_idx] );
                     Log_an_event(src);                         	        	
     	        }
